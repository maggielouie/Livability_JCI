...
(function (factory) {
  ...
  $.fn.truncate = function(options) {
    ...
    $(this).each(function() {
      ...
      var full_node = $(this).hide().addClass('hidden');

      truncated_node.insertAfter(full_node);

      findNodeForMore(truncated_node).append(opts.moreSeparator+'<a href="#showMoreContent">'+opts.more+'</a>');
      if (opts.less)
        findNodeForLess(full_node).append(opts.lessSeparator+'<a class="showless" href="#showLessContent">'+opts.less+'</a>');
  ...
  function truncateNode(node, max_length) {
    var node = $(node);
    var new_node = node.clone().addClass("truncated").empty();
    var truncatedChild;
