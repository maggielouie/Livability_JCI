...
if (isset($node) && is_object($node) && in_array($node->type, array('city', 'area', 'state', 'county'))) {
  if ((!isset($node->field_promoted_city) || $node->field_promoted_city['und'][0]['value'] == 1) && (!empty($node->field_banner_image))) {
    $bannerimage = image_style_url('place_banner_-_1400x453', $node->field_banner_image['und'][0]['uri']);
  }
}
?>
<div class="container<?php if(!is_null($bannerimage)) { print ' place-banner'; } ?>"<?php print $attributes; ?>>
<?php
// $contexts = array_keys(context_active_contexts());
// dpm($contexts);
$header_text = $page['#views_contextual_links_info']['views_ui']['view']->header['area']->options['content'];
if ($page['special_header'] == NULL && arg(0) == 'metro' && arg(1) == NULL) {
  $page['special_header'] = '<span class="city-pin">' . '<i class="icon-pin green no-mobile"></i>' . '<h2>' .  $title . '</h2>' .
  '<span class="county-area no-mobile">' . $header_text . '</span>'.'</span>';
}
?>
...
<?php include(drupal_get_path('theme', 'liv').'/partials/header.php'); ?>
  ...
  <?php if(arg(0) !== 'find-city') : ?>
    <div class="clearfix content shadow-lrb page-header<?php if(!is_null($bannerimage)) { print '" style="background:url(\''.$bannerimage.'\'); background-size:cover;'; } else { print ' padded'; } ?>">
